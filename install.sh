...
set_permissions() {
  ...
  for APPLET in $(cat $TMPDIR/coreutils_modules); do
    case $BIN in
      */bin)
        if [ ! -f "/system/bin/$APPLET" -o -L "/system/bin/$APPLET" ]; then
          $UNITY$BIN/coreutils --coreutils-prog=ln $FLAGS coreutils $APPLET
          $BAKSYM && [ -f "$BIN/$APPLET~" ] && echo "$BIN/$APPLET~" >> $INFO
        fi
      ;;
      *) $UNITY$BIN/coreutils --coreutils-prog=ln $FLAGS coreutils $APPLET
         $BAKSYM && [ -f "$BIN/$APPLET~" ] && echo "$BIN/$APPLET~" >> $INFO
         if [ "$APPLET" == "cp" -o "$APPLET" == "mv" ]; then
           mkdir -p $UNITY/system/bin 2>/dev/null
           cd $UNITY/system/bin
           $UNITY$BIN/coreutils --coreutils-prog=ln $FLAGS $BIN/coreutils $APPLET
           $BAKSYM && [ -f "/system/bin/$APPLET~" ] && echo "/system/bin/$APPLET~" >> $INFO
           cd $UNITY$BIN
         fi
      ;;
    esac
    $BAKSYM && echo "$BIN/$APPLET" >> $INFO
  done
  cd /
